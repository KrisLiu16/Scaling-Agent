...
class WorkerRuntime:
    ...
    async def step(self) -> TurnOutcome:
        if self._rotate_next or (self.rotate_after_turns and self.state.turns_in_session >= self.rotate_after_turns):
            self._rotate_next = False
            await self._rotate()
        wait_s = 0.0 if self._pending_extra else self.continue_after_s
        batch, claims = await self.coord.next_turn(wait_s=wait_s)
        extra = list(self._pending_extra)
        self._pending_extra.clear()
        if not batch.events and not batch.channel_messages and not extra:
        ...
        except Exception as e:  # harness crash: report, rotate, keep the worker alive
        ...
        if self.board_reminder and outcome.ok and not wrote_board and outcome.tool_calls > 0:
